      ...
      <section className="w-full max-w-[73rem] mx-auto pt-16 sm:pt-24 md:pt-32 px-4 sm:px-8 md:px-16 pb-16 sm:pb-20">
        ...
        {(effectiveDate || updatedDate) && (
          <p className="text-white/80 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]">
            {effectiveDate && (
              <span className="block">Effective Date: {effectiveDate}</span>
            )}
            {updatedDate && (
              <span className="block">Last Updated: {updatedDate}</span>
            )}
          </p>
        )}
        ...
        {intro.map((p, idx) => (
          <p
            key={`intro-${idx}`}
            className="mt-4 text-white/80 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]"
        ...
        {sections.map((s, idx) => (
          ...
          >
            ...
            {(s?.paragraphs || []).map((p, pi) => (
              <p
                key={`p-${pi}`}
                className="text-white/80 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]"
                dangerouslySetInnerHTML={{ __html: p }}
              />
            ))}
            ...
            {Array.isArray(s?.bullets) && s.bullets.length > 0 && (
              <ul className="list-disc pl-5 sm:pl-6 mt-3 space-y-1 text-white/80 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]">
                {s.bullets.map((b, bi) => (
                  <li key={`b-${bi}`} dangerouslySetInnerHTML={{ __html: b }} />
                ))}
              </ul>
            )}

            {/* paragraphs2 */}
            {(s?.paragraphs2 || []).map((p, pi) => (
              <p
                key={`p2-${pi}`}
                className="text-white/80 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]"
                dangerouslySetInnerHTML={{ __html: p }}
            ...
            {/* bullets2 */}
            {Array.isArray(s?.bullets2) && s.bullets2.length > 0 && (
              <ul className="list-disc pl-5 sm:pl-6 mt-3 space-y-1 text-white/80 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]">
                {s.bullets2.map((b, bi) => (
                  <li
                    key={`b2-${bi}`}
        ...
        (Array.isArray(contact?.address) && contact.address.length > 0) ? (
          <section className="mt-10">
            <address className="not-italic text-white/80 mt-3 space-y-1 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]">
              {(contact?.address || []).map((line, li) => (
                <div key={`addr-${li}`}>{line}</div>
              ))}
              {contact?.email && (
                <div className="mt-2">
                  Email:{" "}
        ...
        {/* Closing */}
        {Boolean(closingTitle) ||
        (Array.isArray(closing) && closing.length > 0) ? (
          <div className="mt-10 text-white/80 leading-tight text-[1.1rem] sm:text-[1.15rem] md:text-[20px]">
            {closingTitle && (
              <p className="font-medium text-white leading-tight text-[1.2rem] sm:text-[1.3rem] md:text-[23px]">
                {closingTitle}
              </p>
            )}
            {closing.map((p, idx) => (
              <p key={`c-${idx}`} className="mt-2">
                {p}
              </p>
            ))}
          </div>
